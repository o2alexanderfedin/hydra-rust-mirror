...
pub(crate) extern "C" fn new_command(key: i8, name: *mut i8, command: *mut i8)
    -> *mut Command {
    let cmd: *mut Command =
        unsafe { calloc(1 as u64, core::mem::size_of::<Command>() as u64) } as
            *mut Command;
    unsafe { (*cmd).key = key };
...
pub(crate) extern "C" fn command_run(c: *mut Command) -> i32 {
    unsafe {
        if c as *mut () != 0 as *mut () &&
                unsafe { (*c).command } as *mut () != 0 as *mut () {
            return unsafe {
...
pub(crate) extern "C" fn command_add_child(c: &mut Command,
    child: *mut Command) -> () {
    ...
    while unsafe { (*last_child).next } as *mut () != 0 as *mut () &&
            unsafe { (*unsafe { (*last_child).next }).key } as i32 <=
...
pub(crate) extern "C" fn find_command(c: &Command, key: i8) -> *mut Command {
    ...
    while child as *mut () != 0 as *mut () &&
            unsafe { (*child).key } as i32 != key as i32 {
...
pub(crate) extern "C" fn tree_add_command(tree: *mut Command, keys: *mut i8,
    name: *mut i8, command: *mut i8) -> () {
    let mut c: *mut Command =
        find_command(unsafe { &*tree }, unsafe { *keys });
    if unsafe { *unsafe { keys.offset(1 as isize) } } as i32 == 0 {
        if c as *mut () == 0 as *mut () {
            command_add_child(unsafe { &mut *tree },
    ...
    if c as *mut () == 0 as *mut () {
        c =
            new_command(unsafe { *keys }, c"unnamed".as_ptr() as *mut i8,
                core::ptr::null_mut());
...
pub(crate) extern "C" fn print_command(c: &Command) -> i32 {
    unsafe {
        ...
        if !((*c).name).is_null() {
            unsafe {
                fprintf(__stderrp,
                    ...
                    c"\u{1b}[0;34m".as_ptr() as *mut i8, (*c).name,
                    c"\u{1b}[0m".as_ptr() as *mut i8)
            };
            { let __p = &mut lines; let __t = *__p; *__p += 1; __t };
        }
        /// Find longest item
        ...
        while !(child).is_null() {
            let line_width: i32 =
                unsafe { strlen(unsafe { (*child).name } as *const i8) } as
                    i32;
            if line_width > max_line_width { max_line_width = line_width; }
            child = unsafe { (*child).next };
        }
        max_line_width += 5;
        if max_line_width > width { max_line_width = width; }
        ...
        while !(child).is_null() {
            { let __p = &mut current_item; let __t = *__p; *__p += 1; __t };
            if unsafe { (*child).children } != core::ptr::null_mut() {
                unsafe {
                    fprintf(__stderrp,
                        c"%s%c%s %s\u{2794}%s %s+%-*s%s".as_ptr() as *mut i8 as
                            *const i8, c"\u{1b}[0;33m".as_ptr() as *mut i8,
                        unsafe { (*child).key } as i32,
                        c"\u{1b}[0m".as_ptr() as *mut i8,
                        c"\u{1b}[0;35m".as_ptr() as *mut i8,
                        c"\u{1b}[0m".as_ptr() as *mut i8,
                        c"\u{1b}[0;34m".as_ptr() as *mut i8, max_line_width,
                        unsafe { (*child).name }, c"\u{1b}[0m".as_ptr() as *mut i8)
                };
            } else {
                unsafe {
                    fprintf(__stderrp,
                        c"%s%c%s %s\u{2794}%s  %-*s".as_ptr() as *mut i8 as
                            *const i8, c"\u{1b}[0;33m".as_ptr() as *mut i8,
                        unsafe { (*child).key } as i32,
                        c"\u{1b}[0m".as_ptr() as *mut i8,
                        c"\u{1b}[0;35m".as_ptr() as *mut i8,
                        c"\u{1b}[0m".as_ptr() as *mut i8, max_line_width,
                        unsafe { (*child).name })
                };
            }
...
pub(crate) extern "C" fn getch() -> i8 {
    ...
    if unsafe { tcsetattr(0, 0, &raw mut old as *const Termios) } < 0 {
        unsafe {
            perror(c"tcsetattr ICANON".as_ptr() as *mut i8 as *const i8)
        };
    }
...
pub(crate) extern "C" fn read_file(file: *mut i8) -> *mut i8 {
    let f: *mut FILE =
        unsafe {
            fopen(file as *const i8, c"r".as_ptr() as *mut i8 as *const i8)
        };
    if f as *mut () == 0 as *mut () {
        unsafe {
            perror(c"Failed to open file".as_ptr() as *mut i8 as *const i8)
        };
        unsafe { exit(1) };
    }
    ...
    let content: *mut i8 =
        unsafe {
                calloc((file_size + 1 as i64) as u64,
...
pub(crate) extern "C" fn read_field(file: &mut *mut i8, field: *mut i8)
    -> *mut i8 {
    unsafe {
        let key: *mut i8 = *file;
        ...
        if unsafe { **file } as i32 != ',' as i32 {
            unsafe {
                fprintf(__stderrp,
                    c"Found incorrect end after %s, found: %c".as_ptr() as
                            *mut i8 as *const i8, field, unsafe { **file } as i32)
            };
            unsafe { exit(1) };
        }
        unsafe { **file = 0 as i8 };
...
pub(crate) extern "C" fn read_line(c: *mut Command, file: *mut *mut i8)
    -> () {
    let key: *mut i8 =
        read_field(unsafe { &mut *file }, c"key".as_ptr() as *mut i8);
    let name: *mut i8 =
        read_field(unsafe { &mut *file }, c"name".as_ptr() as *mut i8);
...
pub(crate) extern "C" fn clear_lines(count: i32) -> () {
    unsafe {
        ...
        unsafe { setbuf(__stdoutp, 0 as *mut () as *mut i8) };
        {
            let mut i: i32 = 0;
            '__b6: loop {
                if !(i < count) { break '__b6; }
                '__c6: loop { eprint!("[A\r[2K"); break '__c6; }
